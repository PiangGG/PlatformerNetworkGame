...
AFireball::AFireball()
{
	...
	//Ensure that it notifies things of when it collides|确保它在发生碰撞时能通知事物
	...
	Mesh->SetStaticMesh(ConstructorHelpers::FObjectFinder<UStaticMesh>(TEXT("StaticMesh'/Game/Meshes/EditorSphere.EditorSphere'")).Object);
	...
	theMaterial = ConstructorHelpers::FObjectFinder<UMaterial>(TEXT("Material'/Game/Materials/M_Fire.M_Fire'")).Object;
	...
	FireParticles = ConstructorHelpers::FObjectFinder<UParticleSystem>(TEXT("ParticleSystem'/Game/Particles/P_Fire.P_Fire'")).Object;
	...
	//Set our particle system template to the system we imported|将粒子系统模板设置为导入的系统
	FireParticlesComponent->SetTemplate(FireParticles);
	//auto activate on creation|创建时自动激活
...
void AFireball::PostInitializeComponents()
{
	...
	//Set up our physics properties for proper bounce and stuff|设置我们的物理属性以获得适当的反弹和其他东西
	CollisionSphere->SetMassOverrideInKg(NAME_None, 0.1f, true);

	//Will always bounce at same height|总是在同一高度弹跳
